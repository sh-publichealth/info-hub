...
# Total Meal Replacement (TMR) Diabetes Remission Evaluation  
...
## Instrument Summary (Build Checklist)
...
| ema_daily             | **Yes**   | EMA-designated events                                        |
...
> **Eligibility screening for all diabetes intervention work occurs <u>IN A SEPARATE REDCap DATABASE</u> ** 

Eventually, the plan for future project screening is as follows: 
...
3. **This means - for TMR - eligibility screening should occur in a separate REDCap database.** The suggested TMR eligibility instrument for the registry is tabulated below. 
4. The TMR eligibility criteria are:
   ...
   - IF (and only IF) we don't recruit enough participants (50 cases, 50 controls), we will consider diabetes duration 10-years or less.
   - BMI of 25 or greater
5. **Screening Part 1.** Auto populate (Ascension = manual entry):
   - Date of Birth 
...
8. Eligible participants are randomly reordered, then contacted for interest
...
**Eligibility Process Part 2.** If eligible from *Screening Part 1*, create report / list of these eligible participants, along with their eligible values. This list will be exported, randomly reordered, then used by the team to ask participants to attend an in-person screening visit. The Table below documents the additional variables collected at this in-person screening visit. 
...
**From here, instruments are housed in the TMR intervention database**
...
Records liver disease variables using the Echosens fibroscanner.
...
==Go to EuroQol website. REDCap instrument available - must complete registration form==
...
==Confirm / Improve this variable set with Chief Economist==
...
### Instrument: ema_daily (REPEATING)

Collects high-frequency ecological momentary assessment data to explore behavioural, emotional, and contextual factors during the intervention.

==Q: Can we confirm the questions used here by the DiRECT study==

==Q: Discuss this with Leeds Beckett - with the extensive Qualitative Assessment, this may not be needed==

==Q: If we do use, could be uploaded via MyCap - potentially==

| Variable | Label | Field type | Coding | Required |
|---|---|---|---|---|
| ema_date | Date of EMA prompt | date | – | Y |
| ema_time | Time of EMA response | time | HH:MM | Y |
| ema_adherence | Dietary adherence | radio | 0–4 | Y |
| ema_craving | Food craving intensity | radio | 0–4 | Y |
| ema_mood | Current mood | radio | 0–4 | Y |
| ema_stress | Current stress level | radio | 0–4 | Y |
| ema_context | Eating context | radio | Home/Work/Social/Other | N |
...
## Appendix N: ema_daily

| Variable      | Collection timepoints    | Detailed analytical use                                      |
| ------------- | ------------------------ | ------------------------------------------------------------ |
| ema_adherence | Daily during EMA windows | Aggregated predictor of weight and HbA1c change; exploratory mediation analyses |
| ema_craving   | Daily                    | Behavioural context covariate                                |
| ema_mood      | Daily                    | Psychosocial covariate                                       |
| ema_stress    | Daily                    | Psychosocial covariate                                       |
| ema_context   | Daily                    | Environmental context descriptor                             |

EMA analyses are **exploratory and hypothesis-generating**, per SAP.

---

## Appendix O: Derived Outcomes (Analysis Only)
...
## Appendix P: Derived Variables

### Definitions, Derivation Rules, and Analytical Use
...
This appendix defines all derived variables a priori and should be read alongside the Statistical Analysis Plan, which specifies how these variables are combined, modelled, and interpreted.
